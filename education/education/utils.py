...
def get_portal_programs():
    ...
    program_list = [
        frappe.get_doc("Program", program) for program in published_programs
    ]
    portal_programs = [
...
def get_quiz(quiz_name, course):
    try:
        ...
        frappe.throw(
            _("Quiz {0} does not exist").format(quiz_name), frappe.DoesNotExistError
        )
        return None
    ...
    questions = [
        {
            ...
            "options": [
                {"name": option.name, "option": option.option}
...
def check_quiz_completion(quiz, enrollment_name):
    ...
    status = (
        False if quiz.max_attempts == 0 else bool(len(attempts) >= quiz.max_attempts)
    )
    score = None
    result = None
    time_taken = None
