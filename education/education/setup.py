...
def setup_ai_messaging():
	...
	try:
		from education.education.doctype.ai_message_template.ai_message_template import create_default_templates
		result = create_default_templates()
		if result.get("success"):
			print(f"✅ Created {len(result.get('created_templates', []))} default message templates")
		else:
...
def setup_roles_and_permissions():
	...
	if not frappe.db.exists("Has Role", {"parent": "Administrator", "role": "AI Manager"}):
		frappe.get_doc({
			"doctype": "Has Role",
...
def create_sample_data():
	...
	if frappe.conf.get("developer_mode"):
		try:
			...
			if not frappe.db.exists("AI Conversation", {"title": "Sample Student Inquiry"}):
