...
class OpenAIService(BaseAIService):
    ...
    def generate_response(self, prompt, system_prompt=None, context=None):
        ...
        try:
            ...
            return {"success": False, "error": f"OpenAI Service Error: {str(e)}"}
...
class MockAIService(BaseAIService):
    ...
    def classify_intent(self, message):
        ...
        if any(word in message_lower for word in ["fee", "payment", "cost", "tuition"]):
            ...
            word in message_lower for word in ["admission", "apply", "enrollment"]
        ):
            return {"intent": "admission_inquiry", "confidence": 0.8}
        elif any(
            word in message_lower for word in ["problem", "issue", "error", "bug"]
        ):
            return {"intent": "technical_support", "confidence": 0.7}
        elif any(
            word in message_lower for word in ["complaint", "complain", "unhappy"]
        ):
            return {"intent": "complaint", "confidence": 0.7}
        elif any(
            word in message_lower for word in ["feedback", "suggestion", "improve"]
        ):
