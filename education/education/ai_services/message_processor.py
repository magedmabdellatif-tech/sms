...
class MessageProcessor:
	...
	def process_message(self, message_doc):
		...
		try:
			...
			# Analyze sentiment
			sentiment_result = self.ai_service.analyze_sentiment(message_doc.message_content)
			...
			if should_respond:
				...
				if template_response:
					...
					response = ai_response.get("response") if ai_response.get("success") else None
			...
			frappe.log_error(f"Message processing error: {str(e)}", "AI Message Processor")
			return {
				"success": False,
				"error": str(e)
			}
	...
	def build_context(self, message_doc):
		...
		try:
			...
			context.update({
				...
				"current_academic_year": frappe.defaults.get_defaults().get("academic_year"),
				"institution_name": frappe.defaults.get_defaults().get("company") or "Institution"
			})
			
			# Add message-specific context
			if message_doc.conversation:
				conversation = frappe.get_doc("AI Conversation", message_doc.conversation)
				context["conversation_type"] = conversation.conversation_type
				context["conversation_status"] = conversation.status
		
		except Exception as e:
			frappe.log_error(f"Context building error: {str(e)}", "AI Message Processor")
	...
	def get_template_response(self, message_content, intent, user_type, context_data):
		...
		try:
			from education.education.doctype.ai_message_template.ai_message_template import get_matching_templates
			...
			frappe.log_error(f"Template response error: {str(e)}", "AI Message Processor")
	...
	def generate_ai_response(self, message_content, intent, user_type, context_data):
		...
		try:
			...
			frappe.log_error(f"AI response generation error: {str(e)}", "AI Message Processor")
			return {
				"success": False,
				"error": str(e)
			}
	...
	def generate_conversation_summary(self, messages):
		...
		try:
			...
			frappe.log_error(f"Conversation summary error: {str(e)}", "AI Message Processor")
	...
	def analyze_conversation_patterns(self, conversation_name):
		...
		try:
			messages = frappe.get_all(
				...
				fields=["message_content", "intent", "sentiment", "feedback_rating", "creation"],
			...
			ratings = [msg.get("feedback_rating") for msg in messages if msg.get("feedback_rating")]
			...
			if len(messages) > 1:
				...
				analysis["conversation_duration"] = duration.total_seconds() / 3600  # hours
			
			return analysis
		
		except Exception as e:
			frappe.log_error(f"Conversation analysis error: {str(e)}", "AI Message Processor")
			return {}
