...
class AIConversation(Document):
    ...
    def notify_escalation(self):
        ...
        managers = frappe.get_all(
            "Has Role", filters={"role": "Education Manager"}, fields=["parent"]
        )
    ...
    def generate_summary(self):
        ...
        try:
            ...
            from education.education.ai_services.message_processor import \
                MessageProcessor
            ...
            frappe.log_error(
                f"Summary Generation Error: {str(e)}", "AI Conversation Summary"
            )
    ...
    def get_conversation_analytics(self):
        ...
        analytics = {
            ...
            "incoming_messages": len(
                [m for m in messages if m.message_type == "Incoming"]
            ),
            "outgoing_messages": len(
                [m for m in messages if m.message_type == "Outgoing"]
            ),
...
def get_user_conversations(user=None, status=None, limit=20):
    ...
    for conv in conversations:
        ...
        if (
            user in participant_ids
            or conv_doc.started_by == user
            or conv_doc.assigned_to == user
        ):
