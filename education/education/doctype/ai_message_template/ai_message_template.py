...
class AIMessageTemplate(Document):
	...
	def validate_template_content(self):
		...
		if variables and not self.variables:
			frappe.msgprint(f"Template contains variables {variables} but no variable definitions provided")
	...
	def matches_intent(self, intent, confidence=None):
		...
		if self.intent_patterns:
			patterns = [p.strip() for p in self.intent_patterns.split('\n') if p.strip()]
	...
	def matches_conditions(self, context_data):
		...
		try:
			...
			frappe.log_error(f"Template condition evaluation error: {str(e)}", "AI Message Template")
			return False
	...
	def update_usage_stats(self):
		...
		frappe.db.set_value("AI Message Template", self.name, {
	...
	def update_feedback(self, rating, success=True):
		...
		if current_usage > 0:
			new_feedback = ((current_feedback * current_usage) + rating) / (current_usage + 1)
		else:
			new_feedback = rating
...
@frappe.whitelist()
def get_matching_templates(message_content, intent=None, user_type=None, context_data=None):
...
def create_default_templates():
	...
	default_templates = [
		...
		},
		{
			...
			"user_type": "Student",
			"variables": json.dumps({
				"fees_link": "Link to fees information"
			})
		},
		{
