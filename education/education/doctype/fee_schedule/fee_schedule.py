...
class FeeSchedule(Document):
    ...
    def calculate_total_and_program(self):
        ...
        for d in self.student_groups:
            ...
            if (
                self.program
                and student_group_program
                and self.program != student_group_program
            ):
                frappe.msgprint(
                    _(
...
def generate_fee(fee_schedule):
    ...
    if error:
        frappe.db.rollback()
        frappe.db.set_value(
            "Fee Schedule", fee_schedule, "fee_creation_status", "Failed"
        )
        frappe.db.set_value("Fee Schedule", fee_schedule, "error_log", err_msg)

    else:
        frappe.db.set_value(
            "Fee Schedule", fee_schedule, "fee_creation_status", "Successful"
        )
