...
class AIMessage(Document):
    ...
    def before_save(self):
        ...
        if (
            ...
            and self.sender_type == "AI"
            and self.processed_at
        ):
            original_message = self.get_original_message()
            if original_message:
                self.response_time = time_diff_in_seconds(
                    self.processed_at, original_message.creation
                )
    ...
    def get_or_create_conversation(self):
        ...
        existing_conversation = frappe.db.get_value(
            "AI Conversation",
            {"participants": ["like", f"%{self.sender}%"], "status": ["!=", "Closed"]},
            "name",
        )
        ...
        conversation = frappe.get_doc(
            {
                ...
                "title": self.subject
                or f"Conversation between {self.sender} and {self.recipient}",
